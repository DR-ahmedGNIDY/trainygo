...
export async function getCoachSubscriptionSummary(coachId: string): Promise<CoachSubscriptionSummary> {
  ...
  if (!Types.ObjectId.isValid(coachId)) {
    return { daysRemaining: null, endDate: null, planName: null, planDurationMonths: null, maxClients: 0, clientCount: 0 };
  }
  const coach = await User.findOne({ _id: coachId, role: "coach" })
    ...
    .populate("coachProfile.currentPlan", "name tier durationMonths")
    .lean();
  if (!coach) return { daysRemaining: null, endDate: null, planName: null, planDurationMonths: null, maxClients: 0, clientCount: 0 };
  ...
  const plan = coach.coachProfile?.currentPlan as unknown as { name?: { ar?: string; en?: string }; durationMonths?: number } | null;
  ...
  return {
    ...
    planDurationMonths: plan?.durationMonths ?? null,
