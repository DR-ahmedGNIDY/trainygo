...
export function CheckinForm({ access }: { access: ClientAccessState }) {
  ...
  return (
    <div className="mx-auto max-w-xl">
      ...
      <Card className={access.frozen ? "pointer-events-none opacity-60" : ""}>
        <CardHeader>
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input type="number" min={0} max={14} step={0.5} value={sleep} onChange={(e) => setSleep(e.target.value)} disabled={access.frozen} />
              </div>
              <div className="space-y-2">
                <Label className="flex items-center gap-1.5"><Droplets className="h-4 w-4 text-muted-foreground" />{L("الماء (لتر)", "Water (L)")}</Label>
                <Input type="number" min={0} max={10} step={0.1} value={water} onChange={(e) => setWater(e.target.value)} disabled={access.frozen} />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Scale10 value={energy} onChange={setEnergy} disabled={access.frozen} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Scale10 value={adherence} onChange={setAdherence} disabled={access.frozen} />
            </div>
            ...
            <Button type="submit" className="w-full gap-2" disabled={saving || access.frozen}>
...
function Scale10({ value, onChange, disabled }: { value: number; onChange: (v: number) => void; disabled?: boolean }) {
  return (
